        ...
        <div className="border-t border-sidebar-muted px-4 py-4">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              <UserAvatar name={currentUser.name} color={currentUser.avatar_color} size="sm" className="border border-sidebar-muted" />
              <div className="min-w-0">
                <p className="text-xs text-sidebar-foreground truncate font-semibold">{currentUser.name}</p>
                <p className="text-[10px] text-sidebar-foreground/50 capitalize">{currentUser.role}</p>
              </div>
            </div>
            <button
              onClick={logout}
              className="flex items-center gap-1.5 px-2 py-1 border border-primary rounded text-[10px] font-medium text-sidebar-foreground/80 hover:bg-sidebar-muted transition-colors shrink-0"
            >
